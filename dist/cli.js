...
const PUBLISH_ADVISORY = "publish-clean uses pnpm pack intentionally; npm, yarn, and bun pack do not reliably normalize pnpm workspace/catalog manifests.";
...
function warnIfNonPnpmLifecycle() {
    const userAgent = process.env.npm_config_user_agent;
    if (!userAgent || userAgent.startsWith("pnpm/"))
        return;
    console.warn(`${PUBLISH_ADVISORY} Detected lifecycle user agent: ${userAgent}`);
}
function assertCleanGit(packageDir, skip) {
    if (skip)
        return;
    try {
        const output = run("git", ["status", "--porcelain", "--", "."], packageDir).trim();
        if (output)
            throw new PublishCleanError(`Source package has uncommitted changes:\n${output}`);
    }
    catch (error) {
        if (error instanceof PublishCleanError)
            throw error;
...
function stripManifest(pkg, extraDevFields) {
    ...
    if (scripts) {
        ...
        if (Object.keys(kept).length > 0)
            stripped.scripts = kept;
        else
            delete stripped.scripts;
    }
    return stripped;
}
function assertNoMonorepoProtocols(pkg) {
    const failures = [];
...
function assertFilesField(pkg, skip) {
    if (skip)
        return;
...
function validatePackedFiles(files, skipSuspicious) {
...
function collectDeclaredPaths(value, out) {
    if (typeof value === "string") {
        if (value.startsWith("./"))
            out.push(value.slice(2));
        return;
    }
...
function assertDeclaredFiles(pkg, packageDir) {
    const paths = [];
    for (const field of [
        "main",
        "module",
    ...
    if (missing.length > 0)
        throw new PublishCleanError(`Manifest declares files missing from packed artifact:\n${missing.join("\n")}`);
}
...
async function packAndClean(packageDir, opts) {
    requireTool("pnpm");
    requireTool("tar");
    ...
    const extraDevFields = Array.isArray(config.devFields)
        ? config.devFields.filter((field) => typeof field === "string")
        : [];
    ...
    const packedName = run("pnpm", ["pack", "--pack-destination", root], packageDir)
        .trim()
        .split("\n")
        .at(-1);
    if (!packedName)
    ...
    const extra = process.argv.includes("--")
...
async function main() {
    ...
    const result = await packAndClean(packageDir, {
        ...
        registry: typeof parsed.values.registry === "string"
            ? parsed.values.registry
            : null,
        skipFileCheck: parsed.values["skip-file-check"] === true,
    });
    if (!parsed.values["dry-run"] && !parsed.values["guard-only"])
