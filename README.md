...
pnpm pack
    │
    ▼
rewrite package.json inside the tarball
    │
    ▼
re-read it from disk and check it
    │
    ├─▶ unrepaired, and it leaks
    │   or breaks a consumer
    │   └─▶ exit 1
    │
    └─▶ repaired, or only reported
        └─▶ npm publish <that exact file>
```
