...
- [ ] Manifest and consumer behavior — basis: cleaning must preserve useful consumer behavior and reject broken package references.
  ...
  - [ ] Verify the completed manifest surface, update inline rationale and consumer docs, and commit. Node exposed a false pass introduced by the new matcher: main: "*.js" is a literal path. Restricted wildcard expansion to fields that support it; Node-backed regression failed before the fix, then 50 path tests and typecheck passed (96ms test lane).
